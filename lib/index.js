...
let HTMLHyImgElement = class HTMLHyImgElement extends RxLitElement {
    ...
    triggered($trigger, contentWidth$) {
        ...
        startWith(true));
        combineLatest($url, $trigger2).pipe(switchMap(args => this.fetchImage(...args)), catchError((e) => { if (process.env.DEBUG)
            console.error(e); return $url; }), 
        // tap(() => (this.loading = false)),
        tap((url) => (this.url = url))).subscribe();
    }
